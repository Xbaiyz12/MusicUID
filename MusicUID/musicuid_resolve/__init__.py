...
COLLECTION_RE = re.compile(r"(playlist|album)\?id=(\d+)")
SHORT_HOST = "163cn.tv"


async def expand_short_link(url: str) -> str:
    """Expand a ``163cn.tv`` short link into its full target URL.
...
@sv_resolve.on_regex(r"(music\.163\.com|163cn\.tv)", block=False)
async def resolve_share_link(bot: Bot, ev: Event) -> None:
    ...
    song_match = SONG_ID_RE.search(target)
    if song_match is not None:
        await resolve_song(bot, song_match.group(1))
        return

    collection_match = COLLECTION_RE.search(target)
